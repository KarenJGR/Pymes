...
export default function DashboardAdmin() {
    ...
    useEffect(() => {
        const safePosts = posts.map(({ image, ...rest }) => rest); // elimina base64
        localStorage.setItem("posts", JSON.stringify(safePosts));
    }, [posts]);

    useEffect(() => {
        const safeForums = forums.map(({ image, ...rest }) => rest); // elimina base64
    ...
    return (
        <div className="min-h-screen flex bg-gray-100">
            ...
            <div className="w-72 bg-gradient-to-br from-slate-900 via-slate-800 to-black text-white p-8 flex flex-col shadow-xl">
                ...
                <p className="text-gray-300 mb-8 text-sm">Bienvenido, <span className="font-semibold">{user?.name}</span></p>

                <nav className="flex flex-col gap-2">
                    <button onClick={() => setActiveMenu("publicaciones")} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left transition ${activeMenu === "publicaciones" ? "bg-blue-600 text-white shadow-lg" : "hover:bg-white/10"}`}><FileText size={20} /> Publicaciones</button>
                    <button onClick={() => setActiveMenu("foros")} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left transition ${activeMenu === "foros" ? "bg-blue-600 text-white shadow-lg" : "hover:bg-white/10"}`}><MessageSquare size={20} /> Foros</button>
                    <button className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left transition hover:bg-white/10`}><Users size={20} /> Usuarios</button>
                    <button className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left transition hover:bg-white/10`}><Settings size={20} /> Configuración</button>
                </nav>

                <div className="mt-auto pt-6">
                    <button onClick={logout} className="flex items-center justify-center gap-3 w-full py-3 bg-red-600 hover:bg-red-700 rounded-xl text-white transition shadow-lg"><LogOut size={20} /> Cerrar Sesión</button>
                </div>
            </div>
